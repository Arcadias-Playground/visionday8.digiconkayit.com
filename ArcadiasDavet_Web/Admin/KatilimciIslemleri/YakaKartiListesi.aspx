<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.master" AutoEventWireup="true" CodeBehind="YakaKartiListesi.aspx.cs" Inherits="ArcadiasDavet_Web.Admin.KatilimciIslemleri.YakaKartiListesi" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Admin_Icerik" runat="server">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="header">
                    <h2><strong>Yaka Kartı</strong> Basım Listesi</h2>
                </div>
                <div class="body">
    ...
    <asp:SqlDataSource runat="server" ID="OleDbMailGonderimListesi" ConnectionString='<%$ ConnectionStrings:ArcadiasDavetConnection %>' ProviderName='<%$ ConnectionStrings:ArcadiasDavetConnection.ProviderName %>' SelectCommand="SELECT KatilimciTablosu.AdSoyad, KatilimciTablosu.ePosta, KatilimciTablosu.AnaKatilimciID, KatilimciTipiTablosu.KatilimciTipi, AnaKatilimciTablosu.AdSoyad AS [AnaDavetli], KullaniciTablosu.ePosta AS [KullaniciePosta], YakaKartiBasimTablosu.EklenmeTarihi FROM ( ( ( KatilimciTablosu INNER JOIN YakaKartiBasimTablosu ON KatilimciTablosu.KatilimciID = YakaKartiBasimTablosu.KatilimciID ) INNER JOIN KullaniciTablosu ON YakaKartiBasimTablosu.KullaniciID = KullaniciTablosu.KullaniciID ) INNER JOIN KatilimciTipiTablosu ON KatilimciTablosu.KatilimciTipiID = KatilimciTipiTablosu.KatilimciTipiID ) LEFT JOIN ( SELECT * FROM KatilimciTablosu WHERE AnaKatilimciID IS NULL ) AS AnaKatilimciTablosu ON KatilimciTablosu.AnaKatilimciID = AnaKatilimciTablosu.KatilimciID ORDER BY YakaKartiBasimTablosu.EklenmeTarihi DESC"></asp:SqlDataSource>
</asp:Content>
